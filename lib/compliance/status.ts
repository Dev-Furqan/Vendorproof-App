...
export function getRequirementStatus(requirement: VendorRequirementRecord): ComplianceStatus {
  ...
  if (latestReview && latestReview.status !== "approved") return "deficient";
  if (!requirement.document) return requirement.status === "missing" ? "missing" : "never_responded";
  if (requirement.document.status === "pending_review") return "under_review";
  if (requirement.document.status === "rejected") return "deficient";
  ...
  if (requirement.document.status === "approved" || requirement.status === "compliant") return "compliant";
...
export function summarizeStatuses(statuses: ComplianceStatus[]) {
  return statuses.reduce(
    ...
    { total: 0, compliant: 0, expiring: 0, expired: 0, missing: 0, underReview: 0, deficient: 0 }
  );
}

export function overallStatus(summary: ReturnType<typeof summarizeStatuses>): ComplianceStatus {
  if (summary.deficient > 0 || summary.missing > 0) return "missing";
  if (summary.underReview > 0) return "under_review";
  if (summary.expiring > 0) return "expiring";
  return "compliant";
}
...
export function attachDocumentsToRequirements({
  ...
  documents.forEach((document) => {
    if (!document.vendor_requirement_id) return;
    const sortedVersions = (versionsByDocument.get(document.id) ?? []).sort((a, b) => (b.version_number ?? 0) - (a.version_number ?? 0));
    const sortedReviews = (reviewsByDocument.get(document.id) ?? []).sort(
      (a, b) => new Date(b.reviewed_at ?? b.id).getTime() - new Date(a.reviewed_at ?? a.id).getTime()
    );
...
export function buildVendorComplianceRows({
  ...
  return vendors.map((vendor) => {
    const vendorRequirements = requirements.filter((requirement) => requirement.vendor_id === vendor.id);
    ...
    return {
      ...
      propertyName: firstPropertyId ? propertyNames.get(firstPropertyId) ?? "Unknown property" : "Unassigned",
      ...summary,
